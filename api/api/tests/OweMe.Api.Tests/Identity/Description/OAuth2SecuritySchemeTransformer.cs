...
public class OAuth2SecuritySchemeTransformerTests
{
    ...
    public async Task TransformAsync_WithValidAuthority_AddsBothOAuth2AndBearerSchemesAndRequirements()
    {
        ...
        oauth2Scheme.Flows.Password.Scopes.ShouldNotBeNull();
        ...
        document.Security.Count.ShouldBe(2);
        document.Security.Any(r => r.Keys.Any(k => k.Reference?.Id == "OAuth2")).ShouldBeTrue();
        document.Security.Any(r => r.Keys.Any(k => k.Reference?.Id == "Bearer")).ShouldBeTrue();
    }
    ...
    public async Task TransformAsync_WithNullAuthority_SkipsOAuth2AndOnlyAddsBearer()
    {
        ...
        document.Security.ShouldNotBeNull();
        document.Security.Count.ShouldBe(1);
        document.Security.First().Keys.First().Reference.Id.ShouldBe("Bearer");
    }
    ...
    public async Task TransformAsync_WithInvalidAuthorityUri_SkipsOAuth2AndOnlyAddsBearer()
    {
        ...
        document.Security.ShouldNotBeNull();
        document.Security.Count.ShouldBe(1);
        document.Security.Single().Keys.First().Reference.Id.ShouldBe("Bearer");
    }
}